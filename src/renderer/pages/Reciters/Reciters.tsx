    ...
    <div className="space-y-6 animate-in fade-in duration-300 select-none">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 px-2">
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-xs text-slate-200 focus:outline-none focus:border-brand-500/50"
          >
            <option value="all" className="bg-dark-card text-white">
              جميع الروايات
            </option>
            {riwayat.map((rw) => (
              <option key={rw.id} value={rw.id} className="bg-dark-card text-white">
                {rw.name}
              </option>
            ))}
          </select>
        ...
        <div className="text-xs text-slate-400">
          عدد القراء: <span className="text-brand-400 font-bold">{filteredReciters.length}</span>
        </div>
      </div>
      ...
      {filteredReciters.length === 0 ? (
        <div className="py-20 text-center space-y-3">
          ...
          <p className="text-xs text-slate-400">جرب البحث بكلمات أخرى أو اختر حرفاً مختلفاً</p>
        </div>
      ) : (
