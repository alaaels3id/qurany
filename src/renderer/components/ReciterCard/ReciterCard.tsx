  ...
  if (layout === 'list') {
    return (
      <div
        ...
        className="flex items-center justify-between p-4 rounded-2xl glass-panel transition-all duration-200 group hover:border-brand-500/40 hover:bg-slate-50/50 dark:hover:bg-white/5 cursor-pointer"
      >
        <div className="flex items-center gap-4 min-w-0">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-brand-600 to-emerald-500 dark:from-brand-900 dark:to-emerald-700/60 border border-brand-500/20 flex items-center justify-center text-white dark:text-emerald-300 font-bold flex-shrink-0 group-hover:scale-105 transition-transform shadow-md shadow-brand-500/10">
            <Mic2 className="w-5 h-5" />
          </div>
          <div className="min-w-0">
            <h3 className="font-bold text-base text-slate-800 dark:text-slate-100 font-cairo group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors truncate">
              {reciter.name}
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-1 mt-0.5" title={moshafNames}>
              {moshafNames}
            </p>
          </div>
        </div>
