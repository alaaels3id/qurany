...
export const AudioPlayer: React.FC = () => {
  ...
  if (!currentTrack) {
    return (
      <div className="h-24 glass-player flex items-center justify-between px-8 text-slate-400 select-none border-t border-white/5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-medium text-slate-300">لم يتم اختيار تلاوة بعد</div>
            <div className="text-xs text-slate-400">اختر سورة أو قارئ أو محطة إذاعية للبدء</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <>
      <div className="h-24 glass-player flex items-center justify-between px-8 z-30 relative select-none border-t border-white/5">
        {/* Left: Track Details */}
        <div className="flex items-center gap-4 w-1/4 min-w-[240px]">
          <div className="relative w-14 h-14 rounded-2xl bg-gradient-to-tr from-brand-900 to-emerald-700/80 border border-brand-500/30 flex items-center justify-center text-white shadow-lg shadow-brand-500/10 flex-shrink-0">
            {isRadio ? (
              <RadioIcon className="w-7 h-7 text-emerald-300" />
            ) : (
              <span className="font-amiri font-bold text-2xl text-emerald-200">
          ...
          <div className="truncate">
            <div className="flex items-center gap-2">
              <h4 className="font-bold text-base text-slate-100 font-cairo truncate">
              ...
              {isRadio ? (
                ...
                <>
                  ...
                  {currentTrack.moshafName && (
                    <span className="text-slate-400">• {currentTrack.moshafName}</span>
                  )}
                </>
              )}
            </p>
        ...
        <div className="flex flex-col items-center justify-center max-w-xl w-full px-4">
          <div className="flex items-center gap-5 mb-1.5">
            ...
            <button
              ...
              className="p-2 rounded-xl text-slate-300 hover:text-white disabled:opacity-30 disabled:hover:text-slate-300 transition-transform active:scale-95"
        ...
        <div className="flex items-center justify-end gap-3 w-1/4 min-w-[240px]">
          ...
          {!isRadio && (
            <div className="relative">
              ...
              {showSpeedMenu && (
                <div className="absolute bottom-full mb-2 left-0 w-28 bg-dark-card border border-white/10 rounded-2xl shadow-xl p-1 z-50">
                  <div className="text-[10px] text-slate-400 px-3 py-1 font-semibold">السرعة</div>
      ...
      {showQueue && !isRadio && (
        <div className="absolute bottom-24 left-6 w-96 max-h-[480px] bg-dark-card/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl z-40 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-3 duration-200">
          <div className="p-4 border-b border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <ListMusic className="w-4 h-4 text-brand-400" />
              <h3 className="font-bold text-sm text-slate-100 font-cairo">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-2.5 rounded-xl text-right transition-all group ${
                      isCurrent
                        ? 'bg-brand-500/15 text-brand-400 border border-brand-500/20 font-bold'
                        : 'text-slate-300 hover:bg-white/5'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                    ...
                    {isCurrent && isPlaying && (
                      <span className="w-2 h-2 rounded-full bg-brand-400 animate-ping mr-2" />
                    )}
                  </button>
                );
              })
