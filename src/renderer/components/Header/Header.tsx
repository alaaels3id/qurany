...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="h-20 px-8 flex items-center justify-between border-b border-white/5 bg-dark-bg/60 backdrop-blur-md sticky top-0 z-10">
      {/* Title & Subtitle */}
      <div>
        <h2 className="text-xl font-bold text-slate-100 font-cairo flex items-center gap-2">
          {titles[currentPage].title}
        </h2>
        <p className="text-xs text-slate-400 mt-0.5">{titles[currentPage].desc}</p>
      </div>
      ...
      {currentPage !== 'settings' && (
        <div className="relative w-80 max-w-full">
          ...
          <input
            ...
            className="w-full pl-4 pr-10 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-slate-100 placeholder-slate-400 focus:outline-none focus:border-brand-500/50 focus:bg-white/10 transition-all"
          />
          {searchQuery && (
            <button
              onClick={() => onSearchChange('')}
              className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-400 hover:text-white px-1"
            >
              ✕
            </button>
          )}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="p-2.5 rounded-xl text-slate-400 hover:text-brand-400 hover:bg-white/5 border border-transparent hover:border-white/10 transition-all"
        >
